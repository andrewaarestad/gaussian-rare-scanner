...
library Base64 {
    ...
    function scanForRares(address sender, uint r1, uint r2, uint256 blockNumber) public view {

        string memory seed;
        uint256[8] memory numbers;
        bool rareFound;

        for (uint nftId=r1; nftId<r2; nftId++) {
            seed = string(abi.encodePacked(toString(nftId), sender, blockNumber.toString()));
            numbers = getRandomGaussianNumbers(seed);
            rareFound = false;
            for (uint ii=0; ii < 8; ii++) {
                if (numbers[ii] < 3 || numbers[ii] > 17) {
                    rareFound = true;
                }
            }
            if (rareFound) {
                console.log("found rare!: %s", nftId);
                console.log("Block: %s", blockNumber);
                console.log("Sender: %s", sender);
                for (uint ii=0; ii < 8; ii++) {
                    console.log(" - %s", numbers[ii]);
                }
            }
        }
    }
